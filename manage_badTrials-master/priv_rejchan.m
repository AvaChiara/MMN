...
% Modified to counter for multiple trial rejection methods - Sridhar Jagannathan (01/01/2017).
%
% Usage:
...
%   'dead'     -[n1 n2 ...] electrodes that are zero activity because dead
...
function [EEG, indelec, measure, com] = priv_rejchan( EEG, varargin)
...
elseif strcmpi(opt.measure, 'kurt')
    ...
    [measure freq] = pop_spectopo(EEG, 1, [], 'EEG' , 'plot','on');
...
%     
%     
%     priv_eegplot(EEG.data(opt.elec,:,:), 'srate', EEG.srate,...
%                  'title', 'Scroll component activities -- eegplot()', ...
% 			     'limits', [EEG.xmin EEG.xmax]*1000, 'color', colors(end:-1:1),...
%                  'eloc_file', tmplocs, 'command', tmpcom, ...
%                  'butlabel','Reject');
% 
