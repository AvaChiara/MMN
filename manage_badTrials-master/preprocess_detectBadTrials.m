...
if opts.threshold
    ...
    if  strcmp(EEGlabStruct.setname(6),'s') % for sleep
    threshold.lower = -300;
    threshold.upper = 300;       
    else
        % for 
...
%Collect the parameters from the EEGlabStruct
    ...
    if isfield(EEGlabStruct.reject, 'rejchan') && ~isempty(EEGlabStruct.reject.rejchan)
        test_elec(EEGlabStruct.reject.rejchan) = [];
    end
    
%Do the error detecting and get the format for marking trials in eegplot
    if ~isempty(threshold)
        [EEGlabStruct,ThreshIdx]=pop_eegthresh(EEGlabStruct,1,test_elec,threshold.lower,threshold.upper,starttime,epochLength,0,0);
    ...
    if ~isempty(slope)
        [EEGlabStruct]=pop_rejtrend(EEGlabStruct,1,test_elec,epochTimeFrames,slope.maxval,slope.rVal,1,0,0);
